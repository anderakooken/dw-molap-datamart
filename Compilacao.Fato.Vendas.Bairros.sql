...
--INSERT INTO [dbo].[Fato_Vendas_Bairros]
SELECT * FROM (
SELECT 

tab_b.idEmpresa as EMPRESA,
tab_b.DATA as DATA,
tab_b.idGeo as BAIRRO,
tab_b.QTDPED,
tab_b.QTDPED_CD,
tab_b.QTDPED_SD,
...
FROM (
				...
				SELECT 
					...
					FROM (
						...
						SELECT 
							...
							ped.idEmpresa,
							cli.idCliente,
							CONCAT(cli.geo,', ',cid.cidade,', ',cid.estado) as idGeo,
							CONVERT(DATE, CONCAT(
							...
							FROM [dbo].[Pedidos] ped
								...
								LEFT JOIN [dbo].[Cidades] cid 
									ON cid.idCidade = cli.idCidade
								
					) tab_a GROUP BY tab_a.dataPedido, tab_a.idEmpresa, tipo, idGeo


			) tab_b 

			GROUP BY idEmpresa, idGeo, DATA
	...
	LEFT JOIN (
		...
		SELECT 
			...
			SELECT 
				...
				GROUP BY tab_a.idEmpresa, tab_a.idGeo, tab_a.dataPedido
	) tab_pacotes

		ON (
			tab_pacotes.dataPedido = tab_b.DATA AND 
			tab_pacotes.idEmpresa = tab_b.idEmpresa AND 
			tab_pacotes.idGeo = tab_b.idGeo
		)
		...
		SELECT 
			...
			FROM (
					...
					SELECT 
						...
						GROUP BY idEmpresa, dataPedido, tipo, qualificacao, tipoPedido, idGeo
	...
	GROUP BY idEmpresa, idGeo, DATA
		...
		ON (
			...
			tab_vndTpCliente.idGeo = tab_b.idGeo
			)
	...
	LEFT JOIN (
		...
		-- TOTAL POR TIPO DE PAGAMENTO
			...
			-- PEDIDOS
				...
				GROUP BY tab_ped.idEmpresa, tab_ped.idGeo, tab_ped.dataPedido, tab_ped.pagamento

			)tab_pagto

			GROUP BY tab_pagto.idEmpresa, tab_pagto.idGeo, tab_pagto.dataPedido
