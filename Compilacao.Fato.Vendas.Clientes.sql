...
--INSERT INTO [dbo].[Fato_Vendas_Clientes]
SELECT * FROM (
SELECT 
...
FROM (
		...
		SELECT 
			...
			FROM (
				...
				SELECT 
					...
					) tab_a GROUP BY tab_a.dataPedido, tab_a.idEmpresa, tipo, idCliente


			) tab_b 

			GROUP BY idEmpresa, idCliente, DATA
	...
	LEFT JOIN (
		...
		SELECT 
			...
			SELECT 
				...
				GROUP BY tab_a.idEmpresa, tab_a.idCliente, tab_a.dataPedido
	) tab_pacotes

		ON (
			tab_pacotes.dataPedido = tab_b.DATA AND 
			tab_pacotes.idEmpresa = tab_b.idEmpresa AND 
			tab_pacotes.idCliente = tab_b.idCliente
		)
		...
		SELECT 
			...
			FROM (
					...
					SELECT 
						...
						GROUP BY idEmpresa, dataPedido, tipo, qualificacao, tipoPedido, idCliente
	...
	GROUP BY idEmpresa, idCliente, DATA
		...
		ON (
			...
			tab_vndTpCliente.idCliente = tab_b.idCliente
			)
	...
	LEFT JOIN (
		...
		-- TOTAL POR TIPO DE PAGAMENTO
			...
			-- PEDIDOS
				...
				GROUP BY tab_ped.idEmpresa, tab_ped.idCliente, tab_ped.dataPedido, tab_ped.pagamento

			)tab_pagto

			GROUP BY tab_pagto.idEmpresa, tab_pagto.idCliente, tab_pagto.dataPedido
		...
		ON (
			...
			total_pagto.idCliente = tab_b.idCliente)

	LEFT JOIN [dbo].[Clientes] cli ON cli.idCliente = tab_b.idCliente
